...
local DISCORD = "nick:kabusxrd"
local SCRIPT_URL = "https://raw.githubusercontent.com/kabusxd/egglua/main/work.lua"
...
local brand = make("TextLabel", {
    ...
    BackgroundTransparency = 1,
    RichText = true,
    Text = 'SHARD<font color="#A874FF"></font>',
...
local headline = make("TextLabel", {
    ...
    BackgroundTransparency = 1,
    RichText = true,
    Text = 'Developed by <font color="#6C8CFF">kabusxrd</font>. Press the button to load the script.',
    Font = Enum.Font.GothamMedium,
    TextSize = 19,
    LineHeight = 1.22,
...
button.MouseButton1Click:Connect(function()
    ...
    if success then
        ...
        btnLabel.Text = "❌ ERROR"
        status.Text = "FAILED"
        tween(status, { TextColor3 = Color3.fromRGB(255, 100, 100) }, 0.2)
        print("Error loading script: " .. tostring(err))
        task.delay(2.5, function()
            if not closing then
                btnLabel.Text = "LOAD SCRIPT"
                status.Text = "READY"
                busy = false
            end
        end)
